...
class ElementsKit_Widget_Blog_Posts extends Widget_Base
{
    ...
    {
        $deps = ['imagesloaded', 'ekit-masonry', 'ekit-blog-posts'];
        return $deps;
    }

    public function get_style_depends()
    {
        $deps = ['ekit-button','ekit-blog-posts'];
        return $deps;
    }
       ...
       $this->add_control(
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors'  => [
                    '{{WRAPPER}} .ekit-blog-post-masonry-inactive .elementskit-blog-block-post, {{WRAPPER}} .ekit-blog-post-masonry-inactive .elementskit-post-image-card, {{WRAPPER}} .ekit-blog-post-masonry-inactive .elementskit-post-card' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
                'condition' => [
                    'grid_masonry' => '',
                ],
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors'  => [
                    ...
                    '{{WRAPPER}} .post-meta-list > span svg' => 'fill: {{VALUE}}',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors'  => [
                    ...
                    '{{WRAPPER}} .post-meta-list > span:hover svg' => 'fill: {{VALUE}}',
                    '{{WRAPPER}}.ekit-blog-posts--bg-hover .elementskit-post-image-card:hover .post-meta-list > span:hover i' => 'color: {{VALUE}}',
                    '{{WRAPPER}}.ekit-blog-posts--bg-hover .elementskit-post-image-card:hover .post-meta-list > span:hover svg' => 'fill: {{VALUE}}',
                ],
            ]
        );
    ...
    protected function render_raw()
    {
        ...
        if ($ekit_blog_posts_is_manual_selection === 'yes') {
            $default = \ElementsKit_Lite\Utils::array_push_assoc(
                $default,
                'post__in',
                (!empty($ekit_blog_posts_manual_selection  && count($ekit_blog_posts_manual_selection) > 0)) ? $ekit_blog_posts_manual_selection : [-1]
            );
        }
        ...
        // Post Items
        $post_items_class = 'ekit-row post-items';

        if ($grid_masonry === 'yes') {
            $post_items_class .= ' ekit-blog-post-masonry-active';
        } else {
            $post_items_class .= ' ekit-blog-post-masonry-inactive';
        }
        ...
        $this->add_render_attribute(
            ...
            [
                ...
                'class' => $post_items_class,
            ]
        );

        $masonry_columns = $this->get_masonry_columns($settings);
        $masonry = [
            'gutterX'    => $masonry_column_gap['size'] ?? 10,
        ...
        if ($grid_masonry === 'yes'):
            ...
            $this->add_render_attribute('post_items', 'data-masonry-lib-config', wp_json_encode($masonry));
        endif;
...
?>
        <div <?php echo $this->get_render_attribute_string('post_items'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Already escaped by elementor
            ...
            while ($post_query->have_posts()) : $post_query->the_post();
                ...
                ?>
                <div class="post-item <?php echo esc_attr($ekit_blog_posts_column); ?>">
                                ...
                                <?php if ('yes' == $settings['ekit_blog_posts_floating_category']) : ?>
                                    <div class="elementskit-meta-categories">
                                        <span class="elementskit-meta-wraper">
                                            <span><?php echo get_the_category_list('</span><span>'); // phpcs:ignore WordPress.Security.EscapeOutput -- Already escaped by WordPress
                                                    ?></span>
                                        </span>
                                    </div>
                                <?php endif; ?>
